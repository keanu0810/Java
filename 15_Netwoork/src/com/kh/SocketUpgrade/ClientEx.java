...
public class ClientEx {
	...
	private static final int SERVER_PORT = 12345; // 서버가 열어놓은 포트번호 서버 = 클라이언트 포토 같아야함

	public static void main(String[] args) {
		String serverHostname = "localhost"; // 우리집주소 = localhost = 127.0.0.1
		isRunning = true;
		// 서버와 연결이 된다면 연결하고, 서버에 연결되어있는 반문자는 한번 연결하면 끝이긴 하지만
		// 서버에서 강제로 종료하고 다시 접속하려면 다시 여기를 타고 null 값을 보여줌
		Thread serverThread = new Thread(() -> {
			// 서버에서 반문자한테 메세지를 보내는 값으로 처음에는 아무 대화도 없기 때문에 null을 줌
			// 서로 다른 쓰레드에서 동작한다
			ServerToClientThread serverToClientThread = null;

			// 반문자가 서버한테 메세지를 보내는 값으로 처음에는 아무 대화도 없기 떄문에 null을 줌
				...
				if (clientSocket == null) {
					try {
						...
						if (clientSocket.isConnected()) {
							...
							//서버와 성공적으로 접속헸디면, 대화를 시작하기 위한 그릇을 생성해 준 코드
