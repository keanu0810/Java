...
메서드(method)
	...
	ex ) 메서드급 연기 ! 000메소드급 영화 출연

 	Setter : 필드에 작성한 변수를 복사해서 원본 변수는 대신에 값을 변경하고 저장하는 메서드
 		public void set 변수명(자료형 매개변수)  {
 			this.변수 = 매개변수;
 		}
 		
 	Getter : Setter 저장된 변수값을 다른곳에서 사용할 수 있도록 전달해주는 메서드
 		public 자료형 get변수명() {
 			return 변수명;
 		}
 	
 	생성자 : 파일명과 클래스이름과 똑같이 작성을 해줘야하고 
 			객체의 기본 값을 설정하거나 값을 각각 변경해서 사용할는데 쓰이는 메서드
 		...
 		public 클래스이름 (자료형 매개변수, 자료형 매개변수2) {
 			...
 			예를들어 회원가입을 진행할 때 필수로 이름 나이는 넣어주고
 					결혼기념일 광고수신동의는 선택으로 하고 싶다면 힐수로 이름 ~ 주소를 ( ) 넣어줌
 			public 클래스이름 (String이름, String 나이 {
 			this.이름 = 이름;
 			this.나이 = 나이;
 			}
 			
 		void : 어떤 행동을 수행하기 위해 작성, 수행한다음 특별히 전달할 내용이 없을 때 사용
 			   void vocare 라틴어 보이다 비어있다
 			    	특정 행동을 하고 난 다음에 행동을 끝내는 것이지 값을 전달하거나 추가로 어떤 행동을 하지 않는 것
 		...
 		return : 어떤 행동을 실행하고 행동을 한 다음에 행동에 대한 결과를 필요한 곳에 전달
 			...
 			//문자열을 전달 = String
 			 public String 특정행동1 () {
 			 	return"안녕하세요; // 위에 String를 작성했으니 return 다음에 String을 작성
 			 }
 			 
 			 public int 특정행동2() {
 			 	
 			 	return 1; 			//public 옆에 int 를 작성했으니 return 다음에 숫자값 작성
 			 }
