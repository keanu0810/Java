...
Map ├ 	 TreeMap    : 정렬된 데이터를 필요할 경우 사용
	└ LinkedHashMap : 순서를 유지하면서 저장하기 떄문에 순서가 중요한 경우 사용

 Map : 특정 키를 입력하면 해당되는 값을 얻을수 있는 Collection 객체
 	키 : key (=열쇠)
 	값 : value(=값어치가 있는 것)
...
메서드
	...
	containsKey(key)	 : 지정된 키가 존재하는 확인
	containsValue(value) : 지정된 값이 존재하는 확인
	remove(key			 : 지정된 키와 키에 저장된 값을 제거
	ketSet() 			 : 키를 포함하는 집합을 반환
...
public class MapEx {
	public static void main(String[] args) {
		...
		//키는 유일해야함 키의 값은 중복이 될 수 없을 땐 맨 마지막 값으로 덮어씌워짐
		학생들.put(	"철수", 	80);
		
		//학생들 성적 출력하기
		//for-each 학생들에 들어있는 key를 모두 가지고와서 배열해놓은  keySet() 활용
