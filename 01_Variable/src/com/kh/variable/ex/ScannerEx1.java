...
public class ScannerEx1 {
	...
	public static void main(String[] args) {
		/*
		 ...
		 시용자로부터 키보드 입력을 받기 위해
		 Java에서 제공하는 클래스
		 
		 기본으로 구성되어있는 것은 아님
		 import java.util 패키지 표기를 해준 후 사용할 수 있음
		 import 정확한 표기 법 : import java.uyil.Scanner;
		 import 는 가져온다는 의미
		 ...
		 Scanner.next() : int형 정수 1개 입력
		 Scanner.nextLong() : long 형 정수 1개 입력
		 Scanner.nextDouble() : 실수 1개 입력
		 
		 이외에 Scanner 클래스 안에는 여러가지 기능의 존개
		 스태너는 불러온 다음 close를 사용해서 공간을 닫아주는 표시를 해주는게 좋습니다.
		 */
		...
		int num1 = sc.nextInt(); // 다음엔 입력된 정수르 가져와 num1 대입
		
		System.out.print("정수입력 2 : ");
		int num2 = sc.nextInt(); //다음에 입력된 정수를 가져와 num2
		
		// 실수입력 nextDouble을 사용해서 num3 값에 실수 입력 만들기
		...
		// 문제는 없는지 확인 출력을 해주는 습관이 주는 것이 좋음
		String word2 = sc.next();
		
		//***********************************************
		// - next() / nextInt() / nextDouble() 등을 작성한 이후
		// nextLine() 을 작성해야 되는 상황이 오면
		// nextLine() 을 미리 한 번 작성하고 사용하기
		
		sc.nextLine(); // next() 뒤에 남아있는 버퍼값을 Line 안에 넣어줌
		// 그런데 버퍼밧은 우리가 눈으로 보거나 저장을 해야할 이유가 없기 떄문에
		// 앞에 자료형이나 변수형을 사용해서 버퍼를 넣어 저장해야할 이유가
		// 없어 sc.nextLine()으로만 사용
		
		//nextLine()을 호출헤서 남아있던 엔터를 제거
