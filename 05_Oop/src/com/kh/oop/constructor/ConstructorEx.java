...
public class ConstructorEx {
	/* 생성자
	 ...
	 1. 반환값이 없음(return 사용 x, void x 로 위 두개로 선언하지도 않음
	...
	// 필수생성자 괄호 안에 매개변수가 필수로 들어가야 하는 생성자
	// 媒 매(중매) 介 개(개입) 變 변(변하다) 數(숫자가)
	//				(괄호안에 들어가는 변수 매개 변수)
	//              필수로 이름
	...
	public ConstructorEx(int a) {
		...
		name = "이름을 입력해주세여"; //String 이기 떄문에 문자열을 넣어야 함
		phone = "번호를 입력해주세여"; //String 이기 떄문에 문자열을 넣어야 함
		idNo = 				  1; //int 이기 때문에 숫자값을 넣어야 함
		money =  			  1;//int 이기 때문에 숫자값을 넣어야 함
