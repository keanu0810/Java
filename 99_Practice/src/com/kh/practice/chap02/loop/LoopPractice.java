...
public class LoopPractice {
	...
	public void Greeting() {
		...
		// Greeting : 안녕하세요. 인사를 출력하느 매서드
	}
	
	public void  Greeting2() {
		System.out.println("반갑습니다.");
		// Greeting : 받갑습니다, 인사를 출력하느 매서드
	}
	
	public void practice1() {
		//사용자로부터 한 개의 값을 입력바아 1 부터 그 숫자까지의 숫자들은 모두 출력
	...
	public void practice2() {
		//1미만의 숫자가 입력됐다면
		//1이상의 숫자를 입력해주세요가 출력되면서
		// 다시 사용자가 입렵하도록하기
		//입력받기 위해서는 스캐너를 통해서 입력받기
		Scanner sc= new Scanner(System.in);
		int num;
		while(true) {
	...
	public void practice3() {
		...
		if (num < 1) {
			...
			System.out.println(num + " 부터 1까지의 숫자들을 츨력합니다.");
			break;
		}
		//1부터 그 숫자까지의 모든 숫자를 거꾸로 출력하기
		while(num > -1) {
			System.out.println(num + " ");
			num --; // 키보드로 입력받은 숫자에서 -1을 하면서 1이 될 때 까찌 출력
		}
			//System.out.println();
		}
	...
	public void practice4() {
		...
		// 사용자가 값을 입력하도록 하기
		//숫자를 입력받은 숫자부터 1까지출 력하기
		Scanner sc= new Scanner(System.in);
		// 인생에
		int num; // pratice4 안에서 어디에서든 num을 알 수 있도록 바깥으로 호출해준
		// do - while
		// do - while : 조건이 참이 아니어도 무조건 최로 1회는 실행
		...
		do {
			...
			if(num < 1) {
				System.out.println("1 이상의 숫자를 입력해주세요.");
			// 조건에 관계없이 do로 인해 무조건 1회 실행되는 것이기 떄문엔 return이나 bkeak를
			// 작성해주지 않아도 될 경우가 많음
			}
		}while(num >1);
	...
	public void practice5() {
		...
		//1 + 2 + 3 + 4 + 5 + 6 + 7 + 8 =36 
		Scanner sc = new Scanner(System.in);
		System.out.print("점수를 하난 입력하세요 : ");
		int num = sc.nextInt();
		//입력 받은 수의 총 합이 들어갈 바구니\
		int result = 0;
		
		//숫자가 들어왔을 떄 1부터 입력받은 숫자까지 더해서 총합 구하기
		for(int i = 1; i <= num; i++) {
			result += i;
	...
	public void practice7() {
		...
		while(true) {
			...
			//만약에 첫번쨰 숫자와 두번쨰 숫자 모두 1 보다 크거나 같을 경우
			if(firstNum >= 1 && secondNum >= 1) {
				...
				//(   ,   )괄호 안에 숫자를 넣을 떄 숫자 순서는 상관 없음
				// Math에도 random 이 있던데?!
				//Random 파일		Math.random 파일
	...
	public void practice12() {
		...
		while(true) {
			...
			if (operator.equals("+")) {
				...
				//나머지 경우 0으로 나눌 수 없기 떄문에 점수2는 0이 될수 없음
				if(정수2 == 0)
	...
	*/
	...
	for(int line = num; line >= 1 ;   line--){//; 은 for문 안에서 2개만 사용가능
	...
	public void practice15() {
		...
		System.out.print("정수를 입력하세요 : ");
		int size = sc.nextInt();
		// line은 세로롤 된 줄의 번호를 나타냄
		for(int line = 0; line < size; line++) {
			//blank 공백을 출력하기 위한 for문
			// line 번쨰 줄에서는 line개의 공백을 출력하겠다
