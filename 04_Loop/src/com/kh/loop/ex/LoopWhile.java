	...
	/*
	 ...
	 예제코드 1:
	 	while(true) {
	 		...
	 		Systen.out.println("무한반복");
	 	}
	 	
	 	while(false) {
	 		Systen.out.println("싱행하지 못한채로 종료");
	 	}
	 	
	 * */
	
	// 최종으로 싱행하지는 않지만 코드를 작성할 수 있는 메서드
	...
	public static void whilLoopEx() {
		...
		while(true) {
			...
			case 2:
				...
				//break;
				return;
				//break와 return 모두 case를 탈출하는 구문이기 떄문에 
				// 같이 사용할 수 없음
				//break의 경우에는 while이 true일 떄 종료되지 않고 계속 반복
				//return의 경우 while이 true여도 while 코드를 탈풀해서 프로그램을 종료
				
			default :
				System.out.println("잘못된 번호입니다, 번호를 다시 입력해주세요ㅕ.");
			System.out.println("1. 실행 2. 종료");
