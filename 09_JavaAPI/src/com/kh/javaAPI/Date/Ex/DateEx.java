...
public class DateEx {
	public static void main(String[] args) {
		...
		int 월 = 캘린더.get(Calendar.MARCH);// 0부터 가져오기 때문에 +1을 해줘야 함
		int 일 = 캘린더.get(Calendar.DAY_OF_MONTH);
		System.out.println("현재 날짜 : " + 년 + "/" + (월 + 1) + "/" + 일);
		
		/*
		그레고리안 달력은 1년 1월 1일은 무조건 월요일로 정의돼있음
		2024, 4, 23 을 하면 자동으로 2024년 5월 23일로 변경 됨
		2024년 5월 23일은 목요일
		 * */
		
		GregorianCalendar 그레고리안 = new GregorianCalendar();
		GregorianCalendar 그레고리안2 = new GregorianCalendar(2024, 4, 23); // 2024월 5월 23일
		int 날짜 = 그레고리안2.get(Calendar.DAY_OF_WEEK);//일(1) 월(2) 화(3) 수(4) 목(5) 금(6) 토(7)
		System.out.println("날짜 : " + 날짜);// 5월 23일이 목요일이기 떄문에 5라고 나오는 것
		
		/* 4. SimpleDateFormat 년 월 일
		년 : y year
		월 : M Month (분이랑 첫 글자가 m인 것이 같기 떄문에 월은 대문자 M임
		일 : d day
		...
		요일 : E E라는 글자로 표기하자 국제 표준화 기구에서 약속
			만약에 0요일을 쓰고 싶다면 EEEE 를 사용
		*/
		SimpleDateFormat 날짜형식 = new SimpleDateFormat("yyy-mm-dd");
