...
/*
 이미지를 복사하기 위해서는 원본 이미지 파일 결로와
 옮기고 싶은 이미지 파일 경로를 설정
 
 File : 파일을 읽고 쓰고 가져오고 사용하는데 쓰이는 객체
 
 FileInPutStream  :파일의 데이터를 앍어오는데 사용
...
public class ImageCopy {
	public static void main(String[] args) {
		...
		// 자바코드에서 폴더 구분을 작성할 떄는 / 로 작성
		// 모든 파일의 실행은 src로 부터 시작을 함
		// 폴더 경로를 모두 쓰고 나면/를 한 후 파일이름.확장자 작성
		String 원본이미지 = "src/com/kh/imageEx/healing.jpg";// ctrl art 화살표 아래로
		...
		try {
			...
			// 장바구니로 담른 버퍼를 통해서 원본 파일로부터 데이터를 읽어와 복사할 파일에 사용
			...
			System.out.println("이미지 복사완료 되었습니다.");
		} catch (Exception e) {
